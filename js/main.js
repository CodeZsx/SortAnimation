...
var controller = {
    ...
    random: function() {
        ...
        for (var i = 0; i < num; i++) {
            arr.push(parseInt((Math.random() * 20 + 1), 10))
        }
        $('#array').val(arr.join(','))
        container.creatSquares()
    },
    init: function() {
        ...
        function sliderMove(e) {
            ...
            var progress = ((startSliderX + x)*1.0/($('#progressBar').width() - $('#progressBar .slider').width()))*100
            console.log(parseInt(progress))
            // $('#progressBar').text('速度 '+parseInt(progress)+'%')
        }
    }
}
